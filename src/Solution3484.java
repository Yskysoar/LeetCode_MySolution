...
public class Solution3484 {
    public static void main(String[] args) {
        ...
        System.out.println(spreadsheet.getValue("=A2+A2"));
...
/**
 * 使用哈希表，把AX作为key，可以无视判断AX到底存储在什么地方
 */
class Spreadsheet {
    ...
    public void resetCell(String cell) {
        nums.remove(cell);//getValue处理不存在元素取0
    }
    ...
    public int getValue(String formula) {
        ...
        ans += (X.charAt(0) >= 'A' && X.charAt(0) <= 'Z') ? nums.getOrDefault(X, 0) : Integer.parseInt(X);
        ans += (Y.charAt(0) >= 'A' && Y.charAt(0) <= 'Z') ? nums.getOrDefault(Y, 0) : Integer.parseInt(Y);
        return ans;
    }
}
